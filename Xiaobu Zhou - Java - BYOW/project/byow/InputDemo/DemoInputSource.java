...
public class DemoInputSource {
    ...
    public static void main(String[] args) {
        ...
        byow.InputDemo.InputSource inputSource;

        if (inputType == KEYBOARD) {
            inputSource = new byow.InputDemo.KeyboardInputSource();
        } else if (inputType == RANDOM) {
            inputSource = new byow.InputDemo.RandomInputSource(50L);
        } else { // inputType == STRING
            inputSource = new byow.InputDemo.StringInputDevice("HELLO MY FRIEND. QUACK QUACK");
        }
